...
jobs:
  render:
    steps:
      - Render KiCad → *.pdf, *.svg, *.step, *.glb, *-render-*.png
      - Convert board GLB → *.stl (for the OpenSCAD fit-check)
...
KiCad now runs first: `openscad/pb3-fit-check.scad` imports a board STL
converted from the KiCad STEP/GLB export, so that conversion must exist
before the OpenSCAD step runs.
...
Jobs = parallelism or isolation. This is a **pipeline**: KiCad → OpenSCAD → PlantUML → AsciiDoc. Dependencies are linear. Output files feed next step. One job, shared filesystem, done.
