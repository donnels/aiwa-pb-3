...
. *Schematic* (`kicad/pb3-replacement.kicad_sch`) — carries those same
  ...
  FS8205A, TPS62200, Zener). Currently a block/documentation diagram
  without full pin-to-pin wiring — see `kicad/README.adoc`'s "PCB
  Routing" section for the tracked gap.
. *Netlist-derived sim* (Stage 2, not yet implemented) — would generate
  a netlist from that schematic and re-run it in ngspice with real or
  vendor SPICE models, so a pass means the design that gets fabricated
  works, not a stand-in. Blocked until the schematic has real wiring;
  see `kicad/README.adoc` and `containers/ngspice/README.adoc` for what
  unblocks it.
