= Measuring the PB-3 and context
ifndef::flag-book[]
:toc: right
:toclevels: 5
:sectnums:
:sectnumlevels: 5
endif::[]
ifdef::flag-book[]
:imagesdir-save: {imagesdir}
:imagesdir: measurements
endif::[]

This directory holds every measurement photo, caliper log, and derived dimension used to build the PB-3 models.

== Measurement Photos

=== Original PB-3 SLA pack
==== Width [67.73, 67.75]
.Width calipered underside (67.73mm)
image::IMG_9373.jpeg[]

.Width calipered top (67.75mm)
image::IMG_9374.jpeg[]

==== Height [8.19, 8.12]
.Height calipered side (8.19mm)
image::IMG_9375.jpeg[]

.Height calipered side (8.12mm)
image::IMG_9378.jpeg[]

==== Depth [17.65, 17.55]
.Depth calipered top (17.65mm)
image::IMG_9376.jpeg[]

.Depth calipered top (17.55mm)
image::IMG_9377.jpeg[]

=== Battery holder

.battery holder Depth
image::IMG_9379.jpeg[]

.battery holder Width
image::IMG_9380.jpeg[]

.battery holder Height
image::IMG_9381.jpeg[]

.battery holder inside view
image::IMG_9382.jpeg[]

.battery holder inside view
image::IMG_9383.jpeg[]

.battery holder inside view
image::IMG_9384.jpeg[]

.battery holder inside view
image::IMG_9385.jpeg[]

== Free Training & Docs

* https://github.com/ad-si/awesome-3d-printing[awesome-3d-printing] —
  curated list of free 3D-printing tools and guides, including the print
  tolerance/calibration references used to trust these caliper
  measurements against a printed test fit.

//END OF DOC
ifdef::flag-book[]
:imagesdir: {imagesdir-save}
endif::[]
